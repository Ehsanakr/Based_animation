...
vec3 outerSkyRender(vec3 ro, vec3 rd) {
    ...
    vec3 sunCol = HSV2RGB(vec3(0.01, 0.5, iClick * (0.0000005 * iResolution.x - (iAnimProgress_3.z * (0.0000002 * iResolution.x) * (1.0 - iAnimProgress_1.x)) - (iAnimProgress_1.x * (0.0000005 * iResolution.x)) + (iAnimProgress_1.z * 0.0002))));

    // Sun add to col
    col += sunCol / pow((1.002 - dot(sunDir, rd)), 1.9);

    // Horizon light modifier
    vec3 gcol = HSV2RGB(vec3(1.0, 0.4, 0.008 - (iAnimProgress_3.z * 0.004 * (1.0 - iAnimProgress_1.x)) - (iAnimProgress_1.x * 0.0075) + (iAnimProgress_1.y * 0.0006)));
    gcol = gcol * 0.3;
...
vec3 anim_1(vec3 col, vec3 ro, vec3 rd) {
    ...
    vec3 gcol = HSV2RGB(vec3(1.0, 0.8, 0.006 * (1.0 - iAnimProgress_3.z)));
...
void main() {
    ...
    if(iResolution.x > 1250.0 && iResolution.x > iResolution.y) {
        ...
        ro = vec3(0.0 - iAnimProgress_1.x * 2.0 + iAnimProgress_1.z * 2.0 - iAnimProgress_2.x * 0.0,
            //Camera location up - down
        0.5 + (iAnimProgress_3.z * 2.0 * (1.0 - iAnimProgress_1.x)),
            //Camera location forth - back
        -5.0 - iAnimProgress_1.y * 1.5 + iAnimProgress_1.z * 0.5 - iAnimProgress_2.x * 6.6 + iAnimProgress_2.y * 13.5);
            // Camera look right - left
        la = vec3(0.0 - iAnimProgress_1.x * 2.0 - iAnimProgress_1.y * 0.6 + iAnimProgress_1.z * 0.5 + iAnimProgress_2.x * 2.1,
            // Camera look up - down
        1.75 - iAnimProgress_1.z * 0.5 - iAnimProgress_2.x * 1.5 - iAnimProgress_2.y * 1.1,
        ...
        ro = 1.0 * vec3(0.0,
            //Camera location up - down
        0.5 + ((iAnimProgress_3.z * 3.0) * (1.0 - iAnimProgress_1.x)),
            //Camera location forth - back
