...
vec3 outerSkyRender(vec3 ro, vec3 rd) {
    ...
    vec3 sunCol = HSV2RGB(vec3(0.01, 0.5, min(max(iTime * introTime * 0.0003 - 0.00015, 0.0), 0.0015)));
...
void main() {
    ...
    if(isResp) {
        ...
        ro = vec3(0.0 + iAnimProgress_1.x * 1.4,
        ...
        la = vec3(0.0 + iAnimProgress_1.x * 1.4,
            // Camera look up - down
        1.15 + iAnimProgress_1.x * 0.5,
